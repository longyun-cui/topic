...
//        $this->model = new Table;
    ...
    public function view_edit()
    {
        ...
        if($decode_id == 0)
        {
            ...
            return view('admin.table.edit')->with(['operate'=>'create', 'encode_id'=>$id, 'org'=>$org]);
        }
        else
        {
            $table = Table::find($decode_id);
            if($table)
            {
                unset($table->id);
                return view('admin.table.edit')->with(['operate'=>'edit', 'encode_id'=>$id, 'data'=>$table]);
            }
